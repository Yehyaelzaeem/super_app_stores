...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          toolbarHeight: 80,
          title: BlocConsumer<BranchCubit, BranchState>(
            listener: (context, state) {},
            builder: (context, state) {
              if (branchCubit.branchesModel != null &&
                  branchCubit.branchesModel?.data != null) {
                if (branchCubit.branchesModel!.data!.isNotEmpty) {
                  return DropDownField(
                    ...
                    items: branchCubit.branchesModel!.data!
                        .map((e) =>
              ...
              builder: (context, state) {
                if (profileCubit.profileModel != null) {
                  return Row(
                    ...
                    children: [
                      Text(
                        profileCubit.profileModel?.status == 'available'
                      ...
                      CustomSwitch(
                        ...
                        value: profileCubit.profileModel?.status == 'available'
                            ? true
                            : false,
                      )
                    ],
        ...
        body: Column(
          children: [
            Expanded(
              child: (profileCubit.profileModel?.store?.storeType?.id ?? 0) == 2
                  ? TabBarWidget(tabs: [
                          ...
                          page: RefreshIndicator(
                            onRefresh: () async {
                              ...
                              PrescriptionCubit cubit =
                                  context.read<PrescriptionCubit>();
        ...
        page: RefreshIndicator(
          ...
          child: Container(
            ...
            children: [
              ...
              Expanded(child: OrdersTabs(isUnderLine: isUnderLine))
            ],
          )),
        ));
  }
  ...
  Widget OrdersTabs({bool isUnderLine = false}) {
    OrdersCubit cubit = OrdersCubit.get();
    return TabBarWidget(isUnderLine: isUnderLine, tabs: [
      TabItemModel(
          label: LocaleKeys.newOrders.tr(),
          page: RefreshIndicator(
            onRefresh: () async {
              await Future.delayed(const Duration(seconds: 1));
          ...
          page: RefreshIndicator(
            onRefresh: () async {
              ...
              cubit.getAcceptedOrders(
                  status: 'restaurant_accepted', isReload: true);
            },
      ...
      TabItemModel(
          ...
          page: RefreshIndicator(
            onRefresh: () async {
              await Future.delayed(const Duration(seconds: 1));
              cubit.getOnProgressOrders(
                  status: 'restaurant_order_progress', isReload: true);
            },
      ...
      TabItemModel(
          ...
          page: RefreshIndicator(
            onRefresh: () async {
              await Future.delayed(const Duration(seconds: 1));
              cubit.getCompletedOrders(
                  status: 'restaurant_delivered_driver', isReload: true);
            },
      ...
      TabItemModel(
          ...
          page: RefreshIndicator(
            onRefresh: () async {
              await Future.delayed(const Duration(seconds: 1));
              cubit.getRejectedOrders(
                  status: 'restaurant_rejected', isReload: true);
            },
